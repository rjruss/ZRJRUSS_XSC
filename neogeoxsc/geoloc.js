...
function geocode(geodata) {
/*
//***IMPORTANT update file dqm_geo.xshttpdest in sub-package DQM
//***WITH Correct hostname details for the DQM Service
//***then uncomment this section
	var tok_url = 'DQM/dqm_geo.xsjs?' + geodata;
	var gloc = $.ajax({
		url: tok_url,
		async: false
	}).responseText; // This will wait until you get a response from the ajax request.
	...
	return gloc;
*/
	//FOR TESTING comment out above and use below to RETURN a JSON string --
...
var r1 = new Object();
   ...
   var jsonString= JSON.stringify(r1);
return jsonString;
...
function moveme() {
	var textcheck = "Enter free form address";
	var textcheck2 = "Enter City";
	...
	var oButton3 = new sap.ui.commons.Button("button3", {
		text: "GEOCODE",
		press: function() {
			var gd = "sevent=" + encodeURIComponent(oTextField1.getValue()) + "&link=" + encodeURIComponent(oTextField2.getValue());

			var ret = JSON.parse(geocode(gd));
			lon = ret.LON;
			lat = ret.LAT;

			var pointFeature2 = new ol.Feature(new ol.geom.Point([lon, lat]).transform('EPSG:4326', 'EPSG:3857'));
			var mcoordinate = pointFeature2.getGeometry().getCoordinates();
			//https://gis.stackexchange.com/questions/189462/update-a-features-coordinates-geometry
			//https://gis.stackexchange.com/questions/113956/dynamically-change-location-marker-based-on-users-input
			pointFeature.getGeometry().setCoordinates(mcoordinate);
